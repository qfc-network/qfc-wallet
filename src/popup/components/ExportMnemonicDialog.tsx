...
export default function ExportMnemonicDialog({
  open,
  onClose,
  address,
    ...
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="p-5 space-y-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800">{t.settings.exportMnemonic}</h2>
            <p className="text-sm text-gray-500">{t.settings.mnemonicWarning}</p>
          </div>

          <div className="flex items-start gap-2 p-3 rounded-xl bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm">
            <AlertTriangle size={16} className="mt-0.5" />
            <span>{t.settings.mnemonicWarning}</span>
          </div>
          ...
          {!mnemonic ? (
            ...
            <div className="space-y-3">
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-3">
                <div className="text-xs text-gray-500 mb-1">{t.settings.mnemonic}</div>
